...
@require_admin
async def create_user(user_data: CreateUserRequest, request: Request, current_admin=None):
    # Validate admin domain if creating admin
    if user_data.is_admin and not domain_validator.is_valid_admin_email(user_data.email):
        ...
        raise HTTPException(400, f'Admin users must have email from allowed domains: {", ".join(domain_validator.get_allowed_domains())}')
